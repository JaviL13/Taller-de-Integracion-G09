# Permite modificar rutas y módulos del intérprete de Python
import sys
# Premite interactuar con el sistema de archivos
import os
import pytest                               # Framework de testing
# Crea arrays numéricos para generar los pixeles del GeoTIFF de prueba
import numpy as np
# Lee y escribe archivos GeoTIFF y otros formatos raster geoespaciales
import rasterio
# Calcula la transformación geoespacial a partir de coordenadas reales
from rasterio.transform import from_bounds
# Crea objetos falsos para simular QGIS sin tenerlo instalado en el pipeline
...
# Por esto, se cambiarán esos módulos por un mock falso antes de importar
# el pluggin.
qgis_mock = MagicMock()
...
# Agrega la carpeta raíz al path para que Python encuentre los archivos
# del plugin
...
def geotiff_valido(tmp_path):
    ...
    # Imagen de 10x10 píxeles con valores aleatorios entre 0 y 255
    data = np.random.randint(0, 255, (1, 10, 10), dtype=np.uint8)
    # Cubre lon -70 a -69, lat -20 a -19
    transform = from_bounds(-70, -20, -69, -19, 10, 10)
    with rasterio.open(
        filepath,
        'w',                 # Modo escritura
...
# Crea un archivo .tif con texto plano, que rasterio no puede abrir para
# probar cuando falle
...
# pytest, mocks y fixtures. El código fue revisado, ajustado y validado
# localmente.
