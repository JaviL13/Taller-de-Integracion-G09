...
from qgis.core import (QgsVectorLayer, QgsField, QgsFeature, QgsGeometry,
    QgsProject, QgsVectorFileWriter, QgsCoordinateReferenceSystem, QgsCoordinateTransformContext,
)
...
class AnnotationManager:
    #Crea y gestiona la capa de anotaciones y guarda cada anotación en un GeoPackage local.

    def __init__(self, gpkg_path, crs):
        self.gpkg_path = gpkg_path #Ruta a archivo .gpkg donde se guardan anotaciones del usuario
    ...
    def _get_or_create_layer(self):
        #Busca la capa annotations en el proyecto y si no existe la crea y registra en QGIS
        ...
        provider.addAttributes([ #Columnas de la tabla
            QgsField("status",    QVariant.String),   # pending, approved o rejected
        ...
        layer.renderer().symbol().setColor(
            __import__('qgis.PyQt.QtGui', fromlist=['QColor']).QColor(0, 120, 255, 80)
        )
