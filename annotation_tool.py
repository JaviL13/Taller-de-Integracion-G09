...
from qgis.gui import QgsMapTool, QgsRubberBand #Con esto se pueden sontruir las geométrias con los clics del mouse
...
class PolygonDrawTool(QgsMapTool):
    ...
    def __init__(self, canvas, on_polygon_drawn, iface):
        ...
        self.on_polygon_drawn = on_polygon_drawn #Función que recibre un QgsGeometry cuando se completa el polígono
        self.iface = iface
        self.points = []  # lista de QgsPointXY que el usuario va haciendo clic

        # RubberBand es una línea visual que se dibuja mientras el usuario hace clic
    ...
    def canvasPressEvent(self, event):
        ...
        point = self.toMapCoordinates(event.pos())  # convierte píxeles a coordenadas reales
    ...
    def _finalizar_poligono(self): 
        ...
        self._limpiar()
        self.on_polygon_drawn(geometry)  # llama a la función que guarda la anotación
