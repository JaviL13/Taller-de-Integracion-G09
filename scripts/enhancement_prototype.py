...
if __name__ == "__main__":
    ...
    save_png(
        result,
        f"{OUTPUT_DIR}color_ramp_viridis.png",
        "Color Ramp — viridis")
    ...
    save_png(
        result,
        f"{OUTPUT_DIR}color_ramp_RdYlGn.png",
        "Color Ramp — RdYlGn")
    ...
    if data.shape[0] >= 3:
        ...
        save_png(
            result,
            f"{OUTPUT_DIR}decorrelation_stretch.png",
            "Decorrelation Stretch")
    else:
        print(
            f"Solo {data.shape[0]} banda(s) — decorrelation stretch requiere 3")
